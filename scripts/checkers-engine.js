...
function writePaper(origin, cellWidth, boardCanvas){
	var bienBan=""
	...
	d3.select("#redScore").style("display", "none");
	document.getElementById("blackScore").innerHTML = "History: <br>" + bienBan;
	alert(bienBan);
	}
...
function drawBoard(origin, cellWidth, boardCanvas) {
	...
	boardCanvas.append("g")
				...
				//.style("fill", "#9FF7D8")
				//.style("fill", function(d) { if (d.state == red) return "white"; else return "transparent";})
				.style("fill", "transparent")
				
				//.style("fill", function(d) { if (d.state == empty) return "white"; else return "transparent";})
				//.style("stroke", "black")
